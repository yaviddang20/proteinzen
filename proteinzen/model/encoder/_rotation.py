"""
A slightly more numerically stable version of e3nn.o3._rotation
"""
import math
...
def quaternion_to_axis_angle(q, eps=DEFAULT_EPS):
    ...
    angle : `torch.Tensor`
        tensor of shape :math:`(...)`
    """
    angle = 2 * torch.acos(q[..., 0].clamp(-1+eps, 1-eps))
    axis = torch.nn.functional.normalize(q[..., 1:], dim=-1)
    return axis, angle


def matrix_to_axis_angle(R, eps=DEFAULT_EPS):
    ...
    angle = torch.acos(tr.sub(1).div(2).clamp(-1+eps, 1-eps))
    axis = torch.stack(
        [
            R[..., 2, 1] - R[..., 1, 2],
            R[..., 0, 2] - R[..., 2, 0],
...
def xyz_to_angles(xyz, eps=DEFAULT_EPS):
