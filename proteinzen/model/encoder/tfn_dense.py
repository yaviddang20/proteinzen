...
class ProteinDenseAtomicEmbedder(nn.Module):
    def __init__(
        self,
        ns=16,
        nv=4,
    ...
    def _res_edge_features(self, coords):
        ...
        edge_rbf = _rbf(edge_dists, D_max=self.res_D_max, D_count=self.res_num_rbf)
        node_pos = torch.arange(coords.shape[-2], device=coords.device)
        edge_rel_pos = node_pos[..., None] - node_pos[None]
        edge_rel_pos_embed = _node_positional_embeddings(edge_rel_pos, num_embeddings=self.num_pos_embed)
