...
from proteinzen.openfold.utils.rigid_utils import Rotation, Rigid
from proteinzen.openfold.utils.tensor_utils import (
    tree_map,
    tensor_tree_map,
    batched_gather,
)
...
def atom37_to_cg_frames(protein, eps=1e-8, cg_version=1):
    if cg_version == 1:
        _cg = cg
    else:
        raise ValueError(f"unable to find cg version {cg_version}, available versions are [1]")
    aatype = protein["aatype"]
    all_atom_positions = protein["all_atom_positions"]
    all_atom_mask = protein["all_atom_mask"]
