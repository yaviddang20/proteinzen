    ...
    if sep_rot_loss:
        if polar_upweight:
            rot_vf_loss = angle_axis_rot_vf_loss(
                ...
                total_mask,
                norm_scale[..., None],
                seqwise_weight=(res_data["polar_mask"].float() + 1)[..., None]
            )
        else:
            rot_vf_loss = angle_axis_rot_vf_loss(pred_rot_vf, gt_rot_vf, res_data.batch, total_mask, norm_scale[..., None])
    else:
        rot_vf_loss = torch.square(pred_rot_vf - gt_rot_vf).sum(dim=-1)
        if polar_upweight:
            rot_vf_loss = rot_vf_loss * (res_data["polar_mask"].float() + 1)[..., None]
