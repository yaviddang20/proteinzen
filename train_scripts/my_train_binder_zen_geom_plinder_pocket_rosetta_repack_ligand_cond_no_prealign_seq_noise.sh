...
export TORCH_NCCL_DEBUG_INFO_TEMP_FILE=${REPO_ROOT}/outputs/binder_zen_plinder_cond/debug/nccl_trace_rank_
mkdir -p ${REPO_ROOT}/outputs/binder_zen_plinder_cond/debug
...
python ${REPO_ROOT}/train.py \
    ...
    dataset.config="'${REPO_ROOT}/configs/train/data/plinder_pocket_rosetta_repack_ligand_cond.yaml'" \
    +dataset.val_config="'${REPO_ROOT}/configs/train/data/plinder_pocket_rosetta_repack_ligand_cond_val.yaml'" \
    ...
    experiment.lightning.accumulate_grad_batches=6 \
    experiment.checkpointer.train_time_interval=null \
    experiment.checkpointer.every_n_train_steps=500 \
    hydra.run.dir="'${REPO_ROOT}/outputs/binder_zen_geom_plinder_pocket_rosetta_repack_ligand_cond_no_prealign_seq_noise/train'" \
    lmodule.seq_noise_schedule=true
    # NOTE: reference script (my_train_binder_zen_geom_plinder_pocket_ligand_cond_no_prealign_seq_noise.sh)
    # warm-started from a specific prior checkpoint (version_1639285/checkpoints/last.ckpt).
