...
NOTE: You changed se3_transformer.model.norm.NormSE3 bc it was normalizing wrong """
...
class DensityDenoisingLayer(nn.Module):
    ...
    def __init__(self,
        ...
        """
        super().__init__()
        self.density_to_nodes = ConvSE3(
            fiber_in=fiber_density,
            fiber_out=fiber_node,
            fiber_edge=fiber_edge,
            max_degree=max_degree,
            low_memory=low_memory
        )
        self.norm_conv = NormSE3(fiber_node)
    ...
    def forward(
        ...
        node_features = self.norm_conv(
            type_l_add(
                density_to_node_update,
                node_features
            )
        )
...
class SequenceHead(nn.Module):
    ...
    def forward(self, density_features, edge_features, graph, basis):
        ...
        hidden = self.transition(density_scalars)
...
class DensityDenoiser(nn.Module):
    ...
    def __init__(self,
        ...
        self.seq_head = SequenceHead(fiber_density * 2, fiber_edge, scalar_h_dim, low_memory=low_memory)
        self.atom91 = Atom91Head(fiber_density * 2, fiber_edge, scalar_in=scalar_h_dim, low_memory=low_memory)
    ...
    def forward(self, node_features, edge_features, density_features, ts, graph, basis=None):
        ...
        if self.detach_aux_grads:
            f_D = {k: v.detach() for k,v in f_D.items()}
            f_E = {k: v.detach() for k,v in f_E.items()}

        # denoising assistance formulation as inspired by https://arxiv.org/abs/2306.09192
        density_da_features = type_l_cat(f_D, density_features)
        ...
        seq_features = {'0': seq_features.unsqueeze(-1)}
        atom91 = self.atom91(
            density_da_features,
            seq_features,
            f_E,
        ...
        return str_to_int_key(density), seq_logits, atom91['1']
