...
from ligbinddiff.model.modules.layers.node.attention import GraphInvariantPointAttention, InvariantPointMlpAttention
...
class IPAUpdateLayer(nn.Module):
    def __init__(self, c_s, c_z, c_hidden, dropout=0., attn="qkv"):
        super().__init__()
        self.c_s = c_s
        self.c_z = c_z
        ...
        if attn == "qkv":
            ...
            self.ipa = InvariantPointMlpAttention(
                c_s=c_s,
                c_z=c_z,
                c_hidden=c_hidden,
                no_heads=4,
            ...
            raise ValueError("attn must be 'qkv' or 'mlp'")
