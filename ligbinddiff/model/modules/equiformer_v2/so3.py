...
class SO3_Embedding():
    ...
    def to_resolutions(self, lmax_list, mmax_list):
        assert len(lmax_list) == len(mmax_list)

        # shortcut in case we aren't really changing resolutions
        if lmax_list == self.lmax_list:
            if mmax_list == self.mmax_list:
                return self
            else:
                ret = self.clone()
                ret.set_lmax_mmax(lmax_list, mmax_list)
                return ret

        tensor_chunks = []
        offset = 0
        for i, new_lmax in enumerate(lmax_list):
            new_num_coeffs = int((new_lmax + 1) ** 2)
            if i < len(self.lmax_list):
                current_lmax = self.lmax_list[i]
                current_num_coeffs = int((self.lmax_list[i] + 1) ** 2)
                current_resolution_coeffs = self.embedding[:, offset:offset+current_num_coeffs]
                if current_lmax >= new_lmax:  # truncate higher orders
                    tensor_chunks.append(current_resolution_coeffs[:, :new_num_coeffs])
                else:  # add blank values for higher orders
                    num_extra_coeffs = new_num_coeffs - current_num_coeffs
                    tensor_chunks.append(
                        F.pad(current_resolution_coeffs, (0, 0, 0, num_extra_coeffs))
                    )
                offset += current_num_coeffs
            else:  # add new resolutions
                empty_chunk = torch.zeros(
                    self.length,
                    new_num_coeffs,
                    self.num_channels,
                    dtype=self.dtype,
                ...
                tensor_chunks.append(empty_chunk)
        # print([t.shape for t in tensor_chunks])
        # print(lmax_list, mmax_list)
        new_embedding = torch.cat(tensor_chunks, dim=-2)
        ...
        clone.set_embedding(new_embedding)
        clone.set_lmax_mmax(lmax_list, mmax_list)

        return clone

    def condense_resolutions(self):
        lmax_0 = self.lmax_list[0]
        for lmax in self.lmax_list:
            assert lmax_0 == lmax, "all lmaxs must be the same to condense"
        num_coeffs = int((lmax_0+1)**2)

        chunk_sizes = [num_coeffs for _ in self.lmax_list]
        tensor_chunks = torch.split(self.embedding, chunk_sizes, dim=-2)

        out = SO3_Embedding(
            self.length,
            [lmax_0],
            self.num_channels * len(self.lmax_list),
            self.device,
            self.dtype
        )
        out.set_embedding(torch.cat(tensor_chunks, dim=-1))
        return out

    def distribute_resolutions(self, num_resolutions):
        assert len(self.lmax_list) == 1, "we only support distributing resolutions for a single resolution"
        assert self.num_channels % num_resolutions == 0, "can only distribute resolutions to a multiple of the current channel length"
        lmax = self.lmax_list[0]
        tensor_chunks = torch.split(self.embedding, self.num_channels // num_resolutions, dim=-1)

        out = SO3_Embedding(
            self.length,
            [lmax for _ in range(num_resolutions)],
            self.num_channels * len(self.lmax_list),
            self.device,
            self.dtype
        )
        out.set_embedding(torch.cat(tensor_chunks, dim=-2))
        return out

    def get_invariant_features(self, flat=False):
        len_list = [int(lmax + 1) ** 2 for lmax in self.lmax_list]
        start_blocks = [0] + np.cumsum(len_list).tolist()[:-1]
        out = self.embedding[:, start_blocks]
        if flat:
            return out.view(self.length, -1)
        else:
            return out

    def set_invariant_features(self, features):
        if len(features.shape) == 2:
            features = features.view(self.length, self.num_resolutions, -1)
        len_list = [int(lmax + 1) ** 2 for lmax in self.lmax_list]
        start_blocks = [0] + np.cumsum(len_list).tolist()[:-1]
        self.embedding[:, start_blocks] = features
