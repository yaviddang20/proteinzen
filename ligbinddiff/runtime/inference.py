...
def debug_inpaint_inference_loop2(diffuser,
    ...
    for idx, batch in (pbar := tqdm.tqdm(enumerate(sample_sizes))):
        ...
        if save:
            ...
            atom91_to_pdb("".join(["A" for _ in range(batch)]), denoised_bb.numpy(force=True), f"len_{batch}_bb_{idx}")

    return {key: np.mean(losses) for key, losses in epoch_dict.items()}

def debug_inpaint_inference_loop_impute(diffuser,
                           sample_sizes,
                           num_steps,
                           loss_fn,
                           device=None,
    ...
    for idx, batch in (pbar := tqdm.tqdm(enumerate(sample_sizes))):
        ...
        latent_outputs, denoiser_outputs = diffuser.sample_impute(batch, steps=num_steps, show_progress=show_progress, noise_scale=noise_scale, device=device)

        if save:
            denoised_bb = latent_outputs['denoised_bb']
            # denoised_bb = torch.cat([denoised_bb, torch.zeros(batch, 1, 3)], dim=-2)  # add O
            atom91_to_pdb("".join(["A" for _ in range(batch.num_nodes)]), denoised_bb.numpy(force=True), f"{batch.name[0]}_bb_{idx}")
