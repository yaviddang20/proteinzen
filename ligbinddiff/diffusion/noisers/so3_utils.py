...
https://github.com/jasonkyuyim/se3_diffusion/blob/unsupported_refactor/data/so3_utils.py """
...
def Omega(R: torch.Tensor, eps=1e-6):
    r""" Return angle about axis \omega (in the axis-angle rep) from rotation matrix R

    Uses the identity
    1 + 2 \cos(\theta) = trace(R)

    From FrameDiff github, https://github.com/jasonkyuyim/se3_diffusion/blob/unsupported_refactor/data/so3_utils.py
...
def expmap(R0, tangent):
    ...
    if not torch.allclose(skew_sym, -skew_sym.transpose(-1, -2), atol=1e-2,
            rtol=1e-2):
        raise ValueError("in expmap, R0.T @ tangent must be skew symmetric")
