...
class Diffuser(nn.Module, abc.ABC):
    def __init__(self,
        ...
        self.seq_x_0_key = seq_x_0_key
        self.seq_x_0_pred_key = seq_x_0_pred_key
        self.seq_x_t_key = seq_x_t_key
    ...
    def noise(self, data, intermediates, data_coeff, noise_coeff):
        ...
        seq_x_0 = intermediates[self.seq_x_0_key]
        noise = torch.randn_like(seq_x_0)
        data_coeff = data_coeff.squeeze(-1)
        noise_coeff = noise_coeff.squeeze(-1)
        seq_x_t = data_coeff * seq_x_0 + noise_coeff * noise
        intermediates[self.seq_x_t_key] = seq_x_t
    ...
    def score_fn(self, data, intermediates):
        ...
        seq_x_t = intermediates[self.seq_x_t_key]
        t = intermediates['t']
        ...
        alphabar_t = alphabar_t.squeeze(-1)
        seq_x_0_pred = denoiser_output[self.seq_x_0_pred_key]
        seq_score = alphabar_t.sqrt() * seq_x_0_pred - seq_x_t
        seq_score = seq_score / (1 - alphabar_t)

        return score, seq_score, denoiser_output

    def reverse_step(self, data, intermediates, delta_t):
        assert delta_t < 0
        t = intermediates['t']
        score, seq_score, denoiser_output = self.score_fn(data, intermediates)
        x_t = intermediates[self.x_t_key]
        ...
        g = g.squeeze(-1)
        seq_x_t = intermediates[self.seq_x_t_key]
        noise = torch.randn_like(seq_x_t)
        drift = (f * seq_x_t - g**2 * seq_score) * delta_t
        diffusion = g * np.sqrt(np.abs(delta_t)) * noise
        delta_x = drift + diffusion
        seq_x_tm1 = seq_x_t + delta_x

        tm1 = t + delta_t
        return x_tm1, seq_x_tm1, tm1, denoiser_output
    ...
    def sample(self,
        ...
        x_T, seq_x_T = self.sample_prior(num_nodes, device)
        ...
        with torch.no_grad():
            ...
            while (intermediates['t'] > 0).all():
                x_tm1, seq_x_tm1, tm1, denoiser_output = self.reverse_step(data, intermediates, delta_t)
                intermediates['t'] = tm1
                intermediates[self.x_t_key] = x_tm1
                intermediates[self.seq_x_t_key] = seq_x_tm1
        ...
        intermediates[self.x_t_key] = x_T
        intermediates[self.seq_x_0_pred_key] = intermediates[self.seq_x_t_key]
        intermediates[self.seq_x_t_key] = seq_x_T
...
class LatentTwoTrackDiffuser(Diffuser):
    def __init__(self,
                 scheduler,
                 node_lmax_list,
                 edge_channels_list,
        ...
        denoiser = LatentTwoTrackDenoiser(
        ...
        super().__init__(denoiser,
                         ...
                         seq_x_0_key='latent_seq',
                         seq_x_0_pred_key='denoised_latent_seq',
                         seq_x_t_key='noised_latent_seq',)
    ...
    def sample_prior(self, num_nodes, device):
        ...
        seq = torch.randn((num_nodes, self.latent_n_channels), device=device)
    ...
    def forward(self, data, warmup=False, deterministic=False):
        ...
        if deterministic:
            ...
            latent_data[self.seq_x_0_key] = latent_data['seq_mu']
        else:
            ...
            seq_sigma = torch.exp(latent_data['seq_logvar'] * 0.5)
            latent_data[self.seq_x_0_key] = latent_data['seq_mu'] + torch.randn_like(seq_sigma) * seq_sigma
        ...
        seq_x_0 = latent_data[self.seq_x_0_key]
        latent_data[self.x_0_key] = latent_outputs[self.x_0_pred_key]
        latent_data[self.seq_x_0_key] = latent_outputs[self.seq_x_0_pred_key]
        passthrough_outputs = self.decoder(data, latent_data)
        latent_data[self.x_0_key] = x_0 
        latent_data[self.seq_x_0_key] = seq_x_0 
    ...
    def sample(self,
        ...
        latent_outputs, denoiser_output = super().sample(data,
                                               ...
                                               show_progress,
                                               device)
        latent_outputs[self.x_0_key] = so3_randn_like(denoiser_output[self.x_0_pred_key])
        latent_outputs[self.seq_x_0_key] = denoiser_output[self.seq_x_0_pred_key]
        ...
        latent_outputs[self.seq_x_0_key] = encoder_outputs['seq_mu']
