...
class LatentR3InpaintingDiffuser(LatentBaseDiffuser):
    def __init__(self,
                 bb_scheduler,
                 sidechain_scheduler,
                 time_dist,
        ...
        super().__init__(
            ...
            sidechain_scheduler,
            bb_x_0_key='bb',
            bb_x_0_pred_key='denoised_bb',
            bb_x_t_key='noised_bb',
            )
        self.k = k
    ...
    def noise_bb(self, data, sampled_residx, data_coeff, noise_coeff):
        ...
        selected_bb_x_t = selected_bb_x_0 * data_coeff[:, None, None] + bb_noise + noise_coeff[:, None, None]
    ...
    def forward(self, data, warmup=False, deterministic=False):
        ...
        if not warmup:
            ...
            latent_data['noised_residx'] = self.sample_noise_residx(data)
            latent_data['noised_latent_sidechain'] = latent_data['latent_sidechain']
            latent_data['denoised_latent_sidechain'] = latent_data['latent_sidechain']
            latent_data['noised_bb'] = data['bb']
            latent_data['denoised_bb'] = data['bb']
            latent_data['sidechain_loss_weight'] = 1
            latent_data['bb_loss_weight'] = 1
            latent_data['t'] = torch.zeros(data.num_graphs)

        return latent_data, decoded_outputs


class LatentFrameInpaintingDiffuser(LatentBaseDiffuser):
    def __init__(self,
                 bb_scheduler,
                 sidechain_scheduler,
                 time_dist,
