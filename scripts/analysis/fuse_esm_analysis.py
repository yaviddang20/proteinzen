...
cwd = os.getcwd()
os.chdir("/wynton/home/kortemme/alexjli/projects/proteinzen/prelim_experiments/")
...
df_sc = pd.read_csv(os.path.join("framediff_self_cond", tail))
df_sc['condition'] = ["+sparse +batching +self_condition"] * len(df_mod)
df_sc['length'] = [int(name.split("_")[1]) for name in df_sc['name'].tolist()]
