...
RMSD_PROTEIN_BB_CA_TYPE = pyrosetta.rosetta.core.scoring.rmsd_atoms.rmsd_protein_bb_ca
...
def compute_rmsds(design_path, folded_path, pmpnn_paths, fixed_res, fixed_res_chain):
    ...
    pdb_resids = [f"{resid}{chain}" for resid, chain in zip(fixed_res, fixed_res_chain)]
    ...
    for path in pmpnn_paths:
        ...
        rmsd_metric.set_residue_selector(all_res_selector)
        _data['global_ca_rmsd'] = rmsd_metric.calculate(pmpnn_pose)
...
if __name__ == '__main__':
    ...
    for name in parse_dict.keys():
        ...
        global_min_rmsd = sub_df['global_ca_rmsd'].min()
        collapsed_global_df.append(sub_df[sub_df['global_ca_rmsd'] == global_min_rmsd])
