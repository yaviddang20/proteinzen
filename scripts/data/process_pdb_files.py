...
def process_file(file_path: str, write_dir: str, splits: Optional[Dict[str, List[str]]]):
    ...
    for chain_id, chain in struct_chains.items():
        ...
        # chain_dict = data_transforms.make_atom14_masks_np(chain_dict)
        # chain_dict = data_transforms.make_atom14_positions(chain_dict)
    ...
    if splits is not None:
        if pdb_name in splits['train']:
            split = "train"
        elif pdb_name in splits['validation']:
            split = "val"
        elif pdb_name in splits['test']:
            split = "test"
        else:
            split = None
    else:
...
def process_serially(all_paths, write_dir, splits):
    all_metadata = []
...
def main(args):
    ...
    all_file_paths = list(glob.glob(os.path.join(pdb_dir, "*", "*.pdb")))
    ...
    if args.num_processes == 1 or args.debug:
        ...
        with mp.Pool(processes=args.num_processes) as pool:
            all_metadata = list(tqdm.tqdm(pool.imap(_process_fn, all_file_paths), total=len(all_file_paths)))
